...
- 🚫 **ノイズレス** — D問題以降を完全に排除し、今まさに取り組むべき最初の問題だけを表示
- ✋ **手入力ゼロ** — ユーザーIDを入力するだけで、提出履歴を自動取得・自動分類、マーキング
- ⚡ **ラグなし** — 「解けた」を押した瞬間にオプティミスティック更新。API の反映遅延を待たず即座に画面へ反映
...
問題をクリックすると集中モードが起動し、AtCoder の問題ページが別タブで開きます。
解き終わったら「解けた」ボタンを押すだけ。紙吹雪のアニメーションで達成感を演出します。

### 📈 完了率グラフ

Dashboard に A・B・C 問題それぞれの AC 完了率をドーナツグラフで表示。
AC が増えるたびに弧がなめらかに伸びていきます。

### ⚡ オプティミスティック AC

「解けた」を押した瞬間にローカルで即時 AC 反映。kenkoooo API のインデックス遅延（最大数十分）に関わらず、Dashboard・Table が即座に更新されます。
API が実際に AC を返した時点で自動的にローカル状態をクリーンアップします。
